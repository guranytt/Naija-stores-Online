...
function sharedHeader(badgeTitle) {
  return `
  <!-- Logo & Header -->
...
function sharedButton(label, url) {
  return `
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin: 28px 0;">
    <tr>
      <td align="center">
        <a href="${url}" target="_blank" style="background-color: #f97316; color: #ffffff; padding: 14px 34px; border-radius: 14px; text-decoration: none; display: inline-block; font-weight: 700; font-size: 13px; text-align: center; box-shadow: 0 4px 12px rgba(249,115,22,0.15); letter-spacing: -0.1px;">
          ${label}
        </a>
      </td>
    </tr>
...
function buildMasterLayout(badgeTitle, previewText, contentBody) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
const welcome = buildMasterLayout(
  "Welcome to the Family 🛍️",
  "Start exploring thousands of verified top-quality products across marketplaces.",
  `
    <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Hello {{ .Data.name || 'Shopper' }},</h3>
    ...
    ${sharedButton("Start Shopping Now", "{{ .SiteURL }}")}
  `
);
fs.writeFileSync("public/email_welcome.html", welcome);

const orderConf = buildMasterLayout(
  "Order Placed Successfully 📦",
  "Thank you for shopping! We have received your order details.",
  `
    <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Thank You For Your Purchase!</h3>
    <p style="margin: 0 0 14px;">Greetings {{ .Data.customerName }}, we have secured your order <strong>#{{ .Data.orderId }}</strong>.</p>
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 18px; padding: 22px; margin: 24px 0;">
      ...
      <p style="margin: 0; font-size: 13px; color: #475569; line-height: 1.45;">Your payment of ₦{{ .Data.amount }} has been secured in our Escrow.</p>

      <h4 style="margin: 18px 0 6px; color: #0f172a; font-size: 13px; font-weight: 700;">📍 Shipping Destination</h4>
      <p style="margin: 0; font-size: 13px; color: #475569; line-height: 1.45;">{{ .Data.shippingAddress }}</p>

    </div>

    ${sharedButton("Track Order Realtime", "https://www.naijaonlinestores.com.ng/dashboard")}
  `
);
fs.writeFileSync("public/email_order_confirmation.html", orderConf);

const orderShipped = buildMasterLayout(
  "Order Shipped 🚚",
  "Your shipment dispatch has departed safely.",
  `
    <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Your Order Status is Now: Shipped!</h3>
    <p style="margin: 0 0 14px;">Greetings {{ .Data.customerName }}, the platform systems have logged an update for order <strong>#{{ .Data.orderId }}</strong>.</p>
    <p style="margin: 0 0 14px;">Current Status: <span style="background-color: #fff7ed; color: #f97316; padding: 4px 10px; font-weight: bold; border-radius: 6px; font-size: 13px; border: 1px solid #ffedd5;">SHIPPED</span></p>
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px; margin: 24px 0;">
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="font-size: 13px; line-height: 1.5;">
        <tr>
          ...
          <td align="right" style="color: #0f172a; font-weight: bold; font-family: monospace; padding: 4px 0;">{{ .Data.trackingId }}</td>
        </tr>
      </table>
    </div>

    ${sharedButton("Track Shipment Progress", "https://www.naijaonlinestores.com.ng/dashboard")}
  `
);
fs.writeFileSync("public/email_order_shipped.html", orderShipped);
