...
async function getTableColumns(tableName: string): Promise<string[]> {
  ...
  const fallbacks: Record<string, string[]> = {
    ...
    products: ['id', 'vendor_id', 'category_id', 'category', 'name', 'slug', 'description', 'price', 'discount_price', 'stock_quantity', 'featured', 'status', 'created_at'],
...
export async function getSupabaseData<T>(tableName: string, fallbackData: T[]): Promise<{ data: T[]; synced: boolean; error?: string }> {
  try {
    ...
    if (tableName === "products") {
      ...
      queryResult = await supabase.from("products").select("*, product_images(image_url), categories(name)");
