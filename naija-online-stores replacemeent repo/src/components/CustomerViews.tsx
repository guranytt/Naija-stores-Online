...
export default function CustomerViews({
  ...
  useEffect(() => {
    ...
    if (screen === 'shop') {
      const url = activeCategoryTab === 'all' ? '/shop' : `/category/${encodeURIComponent(activeCategoryTab)}`;
  ...
  const filteredProducts = products.filter((product) => {
    let activeCatName = activeCategoryTab;
    const catMatch = categories?.find(c => c.id === activeCategoryTab || c.name === activeCategoryTab);
    if (catMatch) {
      activeCatName = catMatch.name;
    }
    const activeCatLower = activeCatName.toLowerCase();
    const searchLower = searchFilter.toLowerCase();
    
    let matchesCategory = false;
    if (activeCategoryTab.toLowerCase() === "all" || activeCatLower === "all") {
      matchesCategory = true;
    } else {
      const pCatLower = (product.category || "").toLowerCase();
      matchesCategory = pCatLower === activeCatLower || pCatLower.includes(activeCatLower) || activeCatLower.includes(pCatLower);
    }
              ...
              {activeFlashProducts.map((p, idx) => {
                ...
                return (
                  <motion.a
                    ...
                    className="bg-white rounded-2xl border border-orange-100 p-3 flex flex-row h-40 shadow-sm hover:shadow-md cursor-pointer transition-all relative overflow-hidden block"
                  >
                    {/* Left: Image */}
              ...
              {products.slice(4, 12).map((p, idx) => {
                ...
                return (
                  <motion.a
                    ...
                    transition={{ delay: idx * 0.1 }}
                    whileHover={{ y: -4, scale: 1.01 }}
                    className="bg-white rounded-2xl border border-neutral-150 p-3 flex flex-row h-36 shadow-sm hover:shadow-md cursor-pointer transition-all block"
                  >
                    {/* Left: Image */}
                    <div className="w-[35%] h-full bg-neutral-50 rounded-xl overflow-hidden relative shrink-0">
                      {p.image ? (
                ...
                {recentlyViewedProducts.map((p, idx) => {
                  ...
                  return (
                    <motion.a
                      ...
                      whileHover={{ y: -4 }}
                      className="bg-white rounded-2xl border border-neutral-150 p-3 flex flex-row h-28 w-72 shrink-0 shadow-xs hover:shadow-md cursor-pointer transition-all snap-start block"
                    >
                      {/* Left: Image */}
        ...
        >
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-white p-4 border border-neutral-200 rounded-2xl shadow-xs">
            <h1 className="sr-only">
              {activeCategoryTab === "all" ? "All Products" : `${activeCategoryTab} Products`} - NaijaOnlineStores
            </h1>
              ...
              {categories.map((c) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-xs font-bold relative transition-colors duration-200 whitespace-nowrap ${
                    activeCategoryTab === c.name || activeCategoryTab === c.id
                      ? "text-white font-extrabold"
                      : "bg-neutral-100 text-neutral-600 hover:bg-neutral-200"
                  }`}
                >
            ...
            {isSearchLoading ? (
              ...
              return (
                <motion.a
                  ...
                  className="bg-white rounded-2xl border border-neutral-150 overflow-hidden shadow-sm hover:shadow-md group cursor-pointer transition-all flex flex-col justify-between h-auto block"
                  ...
                  <div className="p-4 bg-white border border-neutral-150 rounded-2xl shadow-xs space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="text-left space-y-0.5">
                        ...
                        {/* Turn Vendor Name Into Crawlable Store Link */}
                        <a
                          href={`/vendor/${productVendor.name.toLowerCase().replace(/[^\w ]+/g, "").replace(/ +/g, "-")}`}
                          onClick={(e) => {
                            e.preventDefault();
                            if (onSelectVendor) {
                              onSelectVendor(productVendor.name.toLowerCase().replace(/[^\w ]+/g, "").replace(/ +/g, "-"));
                            }
                            onNavigate("vendor");
                            window.history.pushState({}, "", `/vendor/${productVendor.name.toLowerCase().replace(/[^\w ]+/g, "").replace(/ +/g, "-")}`);
                            window.scrollTo(0, 0);
                          }}
                          className="font-bold text-neutral-900 text-xs leading-none hover:text-orange-600 hover:underline inline-block"
                        >
                          {productVendor.name}
                        </a>
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left w-full">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 pt-1.5 text-xs text-neutral-400 font-semibold select-all">
                    ...
                    {matchedVendor.cacNumber && (
                      <>
                        <span className="hidden sm:inline text-neutral-200">•</span>
                        <span>CAC Reg: <strong className="text-neutral-700">{matchedVendor.cacNumber}</strong></span>
                      </>
                    )}
                    {matchedVendor.bankName && matchedVendor.accountNumber && (
                      <>
                        <span className="hidden sm:inline text-neutral-200">•</span>
                        <span>Bank: <strong className="text-neutral-700">{matchedVendor.bankName} - {matchedVendor.accountNumber}</strong></span>
                      </>
                    )}
                  </div>
                </div>
