...
if (apiKey && apiKey.startsWith("re_")) {
  ...
  console.log("[Email Service] Γ£à Resend API initialised. Live email delivery active.");
} else {
  console.error(
    ...
    "Γòæ  ΓÜá∩╕Å  RESEND_API_KEY is missing or invalid!               Γòæ\n" +
    "Γòæ  All emails will be SILENTLY DROPPED (mock mode).        Γòæ\n" +
    "Γòæ  Add RESEND_API_KEY=re_xxx to your .env or hosting env. Γòæ\n" +
...
async function sendBaseEmail(to: string, subject: string, html: string, type: string, retries: number = 3) {
  if (!resendInstance) {
    console.error(`[EMAIL NOT SENT] RESEND_API_KEY missing ΓÇö dropped: ${type} ΓåÆ ${to}`);
    await logEmail(to, type, subject, "Dropped ΓÇö no API key");
...
function sharedFooterComponent(): string {
  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" align="center" style="color: #64748b; font-size: 12px; line-height: 1.6; text-align: center; padding: 0 16px;">
    <tr>
      <td align="center">
        <p style="margin: 0; font-weight: 700;">Secure Escrow Protection Enabled ≡ƒöÉ</p>
...
export async function sendWelcomeEmail(to: string, name: string) {
  const badge = "Welcome to the Family ≡ƒ¢ì∩╕Å";
  ...
  `;
  return sendBaseEmail(to, "Welcome to Naija Online Stores! Your Marketplace Awaits ≡ƒ¢ì∩╕Å", buildMasterLayout(badge, preview, body), "Welcome Email");
}

export async function sendVendorWelcomeEmail(to: string, name: string) {
  const badge = "Welcome Vendor ≡ƒÜÇ";
  ...
  const body = `
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 18px; margin: 24px 0;">
      <h4 style="margin: 0 0 10px; color: #0f172a; font-size: 13px; font-weight: 700;">≡ƒôê Getting Started</h4>
  ...
  `;
  return sendBaseEmail(to, "Welcome to Naija Online Stores Vendor Network ≡ƒÜÇ", buildMasterLayout(badge, preview, body), "Vendor Welcome Email");
}
...
export async function sendAdminNotificationEmail(newUserEmail: string, role: string, name: string) {
  ...
  const badge = "New Account Alert ≡ƒöö";
...
export async function sendEmailVerification(to: string, name: string, tokenUrlOrLink: string) {
  ...
  const body = `
    <div style="text-align: center; margin-bottom: 24px;">
      <span style="font-size: 48px; line-height: 1; display: block; margin-bottom: 12px;">≡ƒÜÇ</span>
    ...
    <div style="background: linear-gradient(145deg, #fff7ed, #ffedd5); border: 1px solid #fed7aa; border-radius: 16px; padding: 24px; margin-bottom: 24px; text-align: center;">
      ...
      ${sharedButtonComponent("Γ£¿ Activate My Account", verifyLink)}
    </div>
    ...
    <div style="display: flex; align-items: start; gap: 12px; background-color: #f8fafc; border: 1px solid #e2e8f0; padding: 16px; border-radius: 12px; margin-bottom: 20px;">
      <span style="font-size: 20px;">≡ƒöÆ</span>
      <div>
  ...
  `;
  return sendBaseEmail(to, "Γ£¿ Activate Your Naija Online Stores Account!", buildMasterLayout(badge, preview, body), "Email Verification");
}
...
export async function sendPasswordReset(to: string, name: string, tokenUrlOrLink: string) {
  const badge = "Password Reset Request ≡ƒöÉ";
  ...
  `;
  return sendBaseEmail(to, "Reset Your Password - Naija Online Stores ≡ƒöÉ", buildMasterLayout(badge, preview, body), "Password Reset");
}
...
export async function sendOrderConfirmation(
  ...
  const badge = "Order Placed Successfully ≡ƒôª";
    ...
    ? items.map(item => `
        <tr style="border-bottom: 1px solid #e2e8f0;">
          ...
          <td align="right" style="padding: 12px 0; font-size: 14px; font-weight: 700; color: #0f172a;">
            Γéª${((item.price || item.value || 0) * (item.quantity || 1)).toLocaleString()}
          </td>
        </tr>
      `).join("")
  ...
  const body = `
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 18px; padding: 22px; margin: 24px 0;">
      <h4 style="margin: 0 0 14px; color: #0f172a; font-size: 14px; font-weight: 700; border-bottom: 1px solid #e2e8f0; padding-bottom: 8px;">≡ƒôè Order Summary</h4>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-bottom: 14px;">
        ...
        <tr>
          ...
          <td align="right" style="padding: 6px 0 0; font-size: 16px; font-weight: 900; color: #f97316;">Γéª${totalAmount.toLocaleString()}</td>
        </tr>
      </table>

      <h4 style="margin: 18px 0 6px; color: #0f172a; font-size: 13px; font-weight: 700;">≡ƒôì Shipping Destination</h4>
      <p style="margin: 0; font-size: 13px; color: #475569; line-height: 1.45;">${deliveryCity}</p>

      <h4 style="margin: 14px 0 4px; color: #0f172a; font-size: 13px; font-weight: 700;">≡ƒÜÜ Waybill Code:</h4>
  ...
  `;
  return sendBaseEmail(to, `We've Received Your Order #${orderId}! ≡ƒôª`, buildMasterLayout(badge, preview, body), "Order Confirmation");
}
...
export async function sendPaymentSuccessful(to: string, name: string, orderId: string, amount: number) {
  const badge = "Payment Confirmed ≡ƒÄë";
  const preview = `Your payment of Γéª${amount.toLocaleString()} has been safely captured and is secured by Escrow.`;
  ...
  const body = `
    ...
    <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 16px; padding: 20px; text-align: center; margin: 24px 0;">
      ...
      <h2 style="margin: 6px 0; color: #16a34a; font-size: 32px; font-weight: 900;">Γéª${amount.toLocaleString()}</h2>
  ...
  `;
  return sendBaseEmail(to, `Payment Successful for Order #${orderId}! ≡ƒÄë`, buildMasterLayout(badge, preview, body), "Payment Successful");
}
...
export async function sendOrderStatusChange(to: string, name: string, orderId: string, status: string) {
  ...
  const trackingInfoBlock = status.toLowerCase() === "shipped" 
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px; margin: 24px 0;">
      <h4 style="margin: 0 0 12px; color: #0f172a; font-size: 13px; font-weight: 700;">≡ƒÜÜ Waybill Log</h4>
...
export async function sendOrderDeliveredEmail(to: string, customerName: string, orderId: string, reviewLink: string = "https://www.naijaonlinestores.com.ng/dashboard") {
  const badge = "Shipment Delivery Confirmed Γ£à";
  ...
  `;
  return sendBaseEmail(to, `How is your purchase? Review your items from Order #${orderId}! Γ¡É`, buildMasterLayout(badge, preview, body), "Order Delivered");
}
...
export async function sendVendorRegistrationReceived(to: string, vendorName: string) {
  const badge = "Application Received ≡ƒÅ¬";
  ...
  const body = `
    ...
    <p style="margin: 0 0 14px;">Hello ${vendorName}, thank you for applying to sell on Naija Online StoresΓÇöthe country's safest multi-vendor terminal.</p>
  ...
  `;
  return sendBaseEmail(to, "We've Received Your Vendor Application! ≡ƒÅ¬", buildMasterLayout(badge, preview, body), "Vendor Registration Received");
}
...
export async function sendVendorApprovalStatus(to: string, vendorName: string, isApproved: boolean, reason?: string) {
  const badge = isApproved ? "Application Approved ≡ƒÄë" : "Application Update ≡ƒôï";
  ...
  const content = isApproved 
    ...
    <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 16px; padding: 18px; margin: 24px 0; font-size: 13px;">
      <h4 style="margin: 0 0 8px; color: #166534; font-weight: bold;">≡ƒôó Next Steps for Growth</h4>
...
export async function sendVendorNewOrderInfo(to: string, vendorName: string, orderId: string, itemsDetails: string) {
  const badge = "New Customer Order! ≡ƒöö";
  ...
  const body = `
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 18px; padding: 20px; margin: 24px 0; font-size: 13px;">
      <h4 style="margin: 0 0 10px; color: #0f172a; font-weight: 700;">≡ƒôª Ordered Items</h4>
  ...
  `;
  return sendBaseEmail(to, `New Order Alert: Order #${orderId} Received! ≡ƒöö`, buildMasterLayout(badge, preview, body), "New Vendor Order");
}
...
export async function sendLowStockAlert(to: string, vendorName: string, productName: string, stock: number) {
  const badge = "Low Stock Alert ≡ƒôë";
  ...
  `;
  return sendBaseEmail(to, `Low Stock Alert: ${productName} is running low! ≡ƒôë`, buildMasterLayout(badge, preview, body), "Low Stock Alert");
}
...
export async function sendDailySalesSummary(to: string, vendorName: string, totalGMV: number, salesCount: number, pendingOrders: number) {
  const badge = "Daily Sales Summary ≡ƒôê";
  const preview = `Your daily performance snapshot for ${vendorName}: Γéª${totalGMV.toLocaleString()} in orders today.`;
  ...
  const body = `
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 18px; padding: 22px; margin: 24px 0;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="font-size: 14px;">
        ...
        <tr style="border-bottom: 1px solid #f1f5f9;">
          ...
          <td align="right" style="padding: 10px 0; color: #16a34a; font-weight: 800; font-size: 16px;">Γéª${totalGMV.toLocaleString()}</td>
        </tr>
        <tr>
    ...
    ${sharedButtonComponent("Access Seller Dashboard", "https://www.naijaonlinestores.com.ng/admin")}
  `;
  return sendBaseEmail(to, `Your Daily Merchant Performance Digest - ${vendorName} ≡ƒôê`, buildMasterLayout(badge, preview, body), "Daily Sales Summary");
}
...
export async function sendWeeklyPerformanceReport(to: string, vendorName: string, weeklyGMV: number, buyerGrowth: number, bestSellers: string[]) {
  const badge = "Weekly Growth Digest ≡ƒôè";
  ...
  const body = `
    ...
    <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 18px; padding: 22px; margin: 24px 0;">
      <h4 style="margin: 0 0 12px; color: #166534; font-size: 13px; font-weight: 700;">≡ƒôè Performance & Traction</h4>
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="font-size: 13px; color: #14532d;">
        <tr>
          ...
          <td align="right" style="font-weight: 800; font-size: 15px;">Γéª${weeklyGMV.toLocaleString()}</td>
        </tr>
        <tr>
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 18px; padding: 20px; margin: 24px 0;">
      <h4 style="margin: 0 0 10px; color: #0f172a; font-size: 13px; font-weight: 700;">Γ¡É Best Selling Items</h4>
  ...
  `;
  return sendBaseEmail(to, `Your Weekly Merchant Growth Report - ${vendorName} ≡ƒôè`, buildMasterLayout(badge, preview, body), "Weekly Performance Report");
}
...
export async function notifyAdminNewVendor(vendorName: string, email: string) {
  const badge = "Admin Notification: New Vendor Application ≡ƒÜ¿";
  ...
  `;
  return sendBaseEmail("adminnaijastoresonline@gmail.com", `[Admin Alert] New Merchant Application Submitted ≡ƒÜ¿`, buildMasterLayout(badge, preview, body), "Admin Notification");
}
...
export async function notifyAdminNewOrder(orderId: string, amount: number) {
  const badge = "Admin Notification: New Order ≡ƒ¢Æ";
  const preview = `A purchase of Γéª${amount.toLocaleString()} was settled. System verification logs updated.`;
    ...
    <div style="background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 16px; padding: 20px; margin: 24px 0; font-size: 13px;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="line-height: 1.6;">
        ...
        <tr>
          ...
          <td style="color: #16a34a; font-weight: bold;">Γéª${amount.toLocaleString()}</td>
        </tr>
        <tr>
  ...
  `;
  return sendBaseEmail("adminnaijastoresonline@gmail.com", `[Admin Alert] New Marketplace Order #${orderId} Completed ≡ƒ¢Æ`, buildMasterLayout(badge, preview, body), "Admin Notification");
}
...
export async function notifyAdminPaymentReceived(orderId: string, amount: number, ref: string) {
  const badge = "Admin Alert: Payment Approved ≡ƒÆ░";
    ...
    <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; border-radius: 16px; padding: 20px; margin: 24px 0; font-size: 13px;">
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="line-height: 1.6;">
        ...
        <tr>
          ...
          <td style="color: #15803d; font-weight: bold;">Γéª${amount.toLocaleString()}</td>
        </tr>
        <tr>
  ...
  `;
  return sendBaseEmail("adminnaijastoresonline@gmail.com", `[Admin Alert] Payment Transaction Confirmed - Γéª${amount} ≡ƒÆ░`, buildMasterLayout(badge, preview, body), "Admin Notification");
}
...
export async function sendAbandonedCartEmail(to: string, customerName: string, itemsDescription: string) {
  const badge = "Your Shopping Cart is Saved! ≡ƒ¢Æ";
  ...
  const body = `
    ...
    <div style="background-color: #fff7ed; border: 1px solid #ffedd5; border-radius: 16px; padding: 20px; margin: 24px 0; font-size: 13px;">
      <h4 style="margin: 0 0 8px; color: #ea580c; font-weight: bold;">≡ƒ¢ì∩╕Å Pending Safe Items</h4>
  ...
  `;
  return sendBaseEmail(to, "Don't miss out! Complete your checkout on Naija Online Stores ≡ƒ¢Æ", buildMasterLayout(badge, preview, body), "Abandoned Cart Recovery");
}
...
export async function sendRefundProcessed(to: string, name: string, orderId: string, amount: number) {
  const badge = "Refund Processed ≡ƒÆ╕";
  const preview = `Safety system logged a successful escrow reversal of Γéª${amount.toLocaleString()} to your account source.`;
  
  const body = `
    <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Escrow Fund Reversal Complete</h3>
    <p style="margin: 0 0 14px;">Greetings ${name}, we have completed processing your escrow reversal of <strong>Γéª${amount.toLocaleString()}</strong> for Order <strong>#${orderId}</strong>.</p>
...
export async function notifyContactForm(name: string, email: string, message: string) {
  const badge = "Contact Message Received ≡ƒô¼";
  ...
  `;
  await sendBaseEmail("adminnaijastoresonline@gmail.com", `New Contact Form Query from ${name}`, buildMasterLayout("Admin System Log ≡ƒÜ¿", `Contact: ${name}`, adminBody), "Contact Form Summary");
...
export async function previewEmail(type: string, to: string, name: string, orderId: string, amount: number, data: any) {
  switch (type) {
    ...
    case 'customer_signup':
      return buildMasterLayout("Welcome to the Family ≡ƒ¢ì∩╕Å", "Start exploring premium marketplaces", `
        <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Hello ${name},</h3>
    ...
    case 'confirm_email':
      return buildMasterLayout("Action Required Γ£¿", "You're almost there! Activate your account.", `
        <div style="text-align: center; margin-bottom: 24px;">
          <span style="font-size: 48px; line-height: 1; display: block; margin-bottom: 12px;">≡ƒÜÇ</span>
          <h3 style="margin: 0 0 8px; color: #0f172a; font-size: 24px; font-weight: 900; letter-spacing: -0.5px;">Welcome Aboard, ${name}!</h3>
          <p style="margin: 0; color: #64748b; font-size: 15px;">Just one more quick step to unlock the full marketplace magic.</p>
        ...
        <div style="background: linear-gradient(145deg, #fff7ed, #ffedd5); border: 1px solid #fed7aa; border-radius: 16px; padding: 24px; margin-bottom: 24px; text-align: center;">
          ...
          ${sharedButtonComponent("Γ£¿ Activate My Account", "https://www.naijaonlinestores.com.ng/verify?token=preview_token")}
        </div>
      `);
    case 'password_reset':
      return buildMasterLayout("Password Reset Request ≡ƒöÉ", "Regain access securely", `
        <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Account Recovery</h3>
    ...
    case 'order_confirmation':
      return buildMasterLayout("Order Placed Successfully ≡ƒôª", "Your marketplace purchase is safe", `
        ...
        <div style="background-color: #f8fafc; padding: 16px; border-radius: 12px; margin: 18px 0; font-size: 13px;">
          ...
          <strong>Settlement:</strong> Γéª${amount.toLocaleString()}
        </div>
        ${sharedButtonComponent("Track Order Realtime", "#")}
      `);
    case 'payment_confirmation':
    case 'payment_successful':
      return buildMasterLayout("Payment Confirmed ≡ƒÄë", "Secure payment successfully captured", `
        <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 800;">Γéª${amount.toLocaleString()} Captured Successfully</h3>
      ...
      `);
    case 'order_shipped':
      return buildMasterLayout("Order Shipped ≡ƒÜÜ", "Dispacth logistics logs updated", `
    ...
    case 'order_delivered':
      return buildMasterLayout("Shipment Delivery Confirmed Γ£à", "Hope you love your new purchase!", `
      ...
      `);
    case 'review_request':
      return buildMasterLayout("Product Review Request Γ¡É", "Share honest feedback with others", `
    ...
    case 'vendor_application_received':
      return buildMasterLayout("Application Received ≡ƒÅ¬", "Thank you for applying to trade", `
    ...
    case 'vendor_approved':
      return buildMasterLayout("Application Approved ≡ƒÄë", "Merchant account activated", `
    ...
    case 'vendor_rejected':
      return buildMasterLayout("Application Update ≡ƒôï", "Vetting decision summary", `
    ...
    case 'new_order_alert_vendor':
      return buildMasterLayout("New Customer Order! ≡ƒöö", "Fulfill items immediately", `
    ...
    case 'low_stock_alert':
      return buildMasterLayout("Low Stock Alert ≡ƒôë", "Inventory replenishment target", `
    ...
    case 'daily_sales_summary':
      return buildMasterLayout("Daily Sales Summary ≡ƒôê", "Today's commercial performance", `
        <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 850;">Daily business summary</h3>
        <p>Hello ${name}, end-of-day ledger updates report sales of Γéª${amount.toLocaleString()} across 5 items.</p>
        ${sharedButtonComponent("Access Seller Dashboard", "#")}
      `);
    case 'weekly_performance_report':
      return buildMasterLayout("Weekly Growth Digest ≡ƒôè", "Week over week trajectory details", `
    ...
    case 'admin_notification_new_vendor':
      return buildMasterLayout("Application Received ≡ƒÅ¬", "New vendor awaiting verification", `
    ...
    case 'admin_notification_new_order':
      return buildMasterLayout("Admin Notification: New Order ≡ƒ¢Æ", "Marketplace transactional ledger logs", `
    ...
    case 'admin_notification_payment_received':
      return buildMasterLayout("Admin Alert: Payment Approved ≡ƒÆ░", "Paystack transaction verified", `
      ...
      `);
    case 'abandoned_cart':
      return buildMasterLayout("Your Shopping Cart is Saved! ≡ƒ¢Æ", "Complete your checkout securely", `
        <h3 style="margin: 0 0 16px; color: #0f172a; font-size: 20px; font-weight: 850;">Don't Leave This Behind!</h3>
